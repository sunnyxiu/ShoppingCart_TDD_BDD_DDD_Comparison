...
class Cart:
    ...
    def apply_discount(self, discount_code) -> Tuple[bool, str]:
        ...
        # 1. 取得折扣碼物件
        code_obj = get_discount_code(discount_code)
        if not code_obj:
        ...
        # 驗證折價券
        is_valid, message = validate_discount_code(original_total, code_obj)
        
        if not is_valid:
            raise ValueError(message)
        
        # 套用折價券（新的會覆蓋舊的）
        self.applied_discount = code_obj
        return True, "折扣已套用"
    ...
    def discount_amount(self) -> int:
        """取得折扣金額 (相容 UI)"""
        return self.subtotal - self.final_amount